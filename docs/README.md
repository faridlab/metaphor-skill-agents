...
| [4 · Testing](4-testing.md) | Prove it works and stays fast | `test-driven-development`, `business-flow-bdd`, `browser-testing-with-devtools`, `debugging-and-error-recovery`, `performance-optimization`, `chaos-engineering` | `test-writer`, `business-flow`, `debugger`, `perf-analyzer` | `metaphor test --affected`, `metaphor chaos --dry-run` |
